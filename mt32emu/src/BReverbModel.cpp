	...
	while ((numSamples--) > 0) {
		...
		if (tapDelayMode) {
			...
			if (outLeft != NULL) {
				 ...
				 */
				Sample outSample = Synth::clipSampleEx(Synth::clipSampleEx(Synth::clipSampleEx(Synth::clipBit16s((SampleEx)outL1 + SampleEx(outL1 >> 1)) + (SampleEx)outL2) + SampleEx(outL2 >> 1)) + (SampleEx)outL3);
#else
