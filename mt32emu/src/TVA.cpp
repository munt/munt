...
TVA::TVA(const Partial *usePartial, LA32Ramp *useAmpRamp) :
	partial(usePartial), ampRamp(useAmpRamp), muntSystem(&usePartial->getSynth()->mt32ram.system) {
}
...
static int calcBasicAmp(const Tables *tables, const Partial *partial, const MemParams::System *muntSystem, const TimbreParam::PartialParam *partialParam, const MemParams::PatchTemp *patchTemp, const MemParams::RhythmTemp *rhythmTemp, int biasAmpSubtraction, int veloAmpSubtraction, Bit8u expression) {
	int amp = 155;

	if (!partial->isRingModulatingSlave()) {
		amp -= tables->masterVolToAmpSubtraction[muntSystem->masterVol];
		if (amp < 0) {
			return 0;
		}
...
void TVA::reset(const Part *newPart, const TimbreParam::PartialParam *newPartialParam, const MemParams::RhythmTemp *newRhythmTemp) {
	...
	int newTarget = calcBasicAmp(tables, partial, muntSystem, partialParam, patchTemp, newRhythmTemp, biasAmpSubtraction, veloAmpSubtraction, part->getExpression());
	int newPhase;
...
void TVA::recalcSustain() {
	...
	Tables *tables = &partial->getSynth()->tables;
	int newTarget = calcBasicAmp(tables, partial, muntSystem, partialParam, patchTemp, rhythmTemp, biasAmpSubtraction, veloAmpSubtraction, part->getExpression());
...
void TVA::nextPhase() {
	...
	if (!allLevelsZeroFromNowOn) {
		newTarget = calcBasicAmp(tables, partial, muntSystem, partialParam, patchTemp, rhythmTemp, biasAmpSubtraction, veloAmpSubtraction, part->getExpression());
