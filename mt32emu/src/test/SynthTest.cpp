...
static void sendAllNotesOff(Synth &synth, Bit8u channel) {
	synth.playMsgNow(0x7BB0 | channel);
}

static void sendNoteOn(Synth &synth, Bit8u channel, Bit8u note, Bit8u velocity) {
	synth.playMsgNow(0x90 | channel | note << 8 | velocity << 16);
}

// Outputs a pure sine wave with a period of exactly 256 samples at the maximum amplitude in the right channel.
static void sendSineWaveSysex(Synth &synth, Bit8u channel) {
